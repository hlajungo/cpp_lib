#include "alluneed.h"
/* @brief Big num support negative add sub mul div
 * @code_source https://github.com/Limeoats/BigNumber
 * @rewriter&compresser hlajungo&chatGPT */
struct Bn {
  Str n;
  Bn(Str s) : n(s) {}
  ...
  Bn add(Bn o) {
    if (neg() && o.neg()) return abs().add(o.abs()).flip();
  ...
  Bn mul(Bn o) {
    if (neg() != o.neg()) return abs().mul(o.abs()).flip();
    Bn a = abs(), b = o.abs();
