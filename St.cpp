...
struct St{
  ...
  void build(vi& v,int n,int l,int r){
    if(l==r){t[n]=v[l];return;}
    build(v,LRST);build(v,RRST);
    t[n]=max(t[LST],t[RST]); //M
  }
  void lazy(int n,int l,int r){
    if(lz[n]){
      t[n]+=lz[n];
      if(l!=r){lz[LST]+=lz[n];lz[RST]+=lz[n];}
      lz[n]=0;
    }
  }
  void add(int ql,int qr,int val){if(qr<ql||n<qr)return;add(ql,qr,val,1,0,n);}
  void add(int ql,int qr,int val,int n,int l,int r){
    lazy(n,l,r);
    if(!IS_INT(l,r,ql,qr))return;
    if(IS_INC(l,r,ql,qr)){
      t[n]+=val;
      if(l!=r){lz[LST]+=val;lz[RST]+=val;}
      return;
    }
    add(ql,qr,val,LRST);add(ql,qr,val,RRST);
    t[n]=max(t[LST],t[RST]); //M
  }
