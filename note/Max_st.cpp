 ...
 */
struct Max_st {
  inline bool is_intersect(auto&& l1, auto&& r1, auto&& l2, auto&& r2) { return r2 >= l1 && r1 >= l2; }
  inline bool is_include(auto&& l1, auto&& r1, auto&& l2, auto&& r2) { return l1 >= l2 && r2 >= r1; }
  inline int left(int& node) { return node * 2; }
  inline int right(int node) { return node * 2 + 1; }
  inline bool is_leaf(int& l, int& r) { return l == r; }
  int n;
  std::vector<int> tree;
  std::vector<int> lazy;
  // empty construct
  Max_st(auto&& n) : n(n), tree(4 * n, 0), lazy(4 * n, 0) {}
  Max_st(std::vector<int>& v) : n(v.size() - 1), tree(4 * n, 0), lazy(4 * n, 0) { build(v, 1, 0, n); }
  void cout() {
  ...
  void add_interval(auto&& rl, auto&& rr, auto&& val) {
    if (rr < rl) {
  ...
  void build(std::vector<int>& v, auto&& node, auto&& l, auto&& r) {
 ...
 private:
  void try_update_lazy(int node, int l, int r) {
    if (lazy[node] != 0) {
      tree[node] += lazy[node];
      // Has leaf
      if (!is_leaf(l, r)) {
   ...
   */
  void add_interval(auto node, auto l, auto r, auto&& rl, auto&& rr, auto&& val) {
    std::cout << "At node " << node << "\n";
    try_update_lazy(node, l, r);
    if (!is_intersect(l, r, rl, rr)) return;
  ...
  int query(int node, int l, int r, int ql, int qr) {
    // std::cout << "At node " << node << "\n";
    if (!is_intersect(l, r, ql, qr)) return -INT_MAX;
    try_update_lazy(node, l, r);
    if (is_include(l, r, ql, qr)) return tree[node];
...
struct Sum_st {
  ...
  int n;
  std::vector<int> tree;
  void build(std::vector<int>& v, int node, int l, int r) {
    if (is_leaf(l, r)) {
      tree[node] = v[l];
      return;
    }
   ...
   */
  void add_node(const int& idx, const int& val, int node, int l, int r) {
    if (is_leaf(l, r)) {
      tree[node] += val;
      return;
    }
  ...
  int query(int ql, int qr, int node, int l, int r) {
    if (!is_intersect(l, r, ql, qr)) return 0;
    if (is_include(l, r, ql, qr)) return tree[node];
    int mid = (l + r) / 2;
    // Partial include, let left child and right child answer the question
    return query(ql, qr, left(node), l, mid) + query(ql, qr, right(node), mid + 1, r);
  }
 public:
  // empty construct
  Sum_st(auto&& n) : n(n), tree(4 * n, 0) {}
  Sum_st(std::vector<int>& v) { build(v, 1, 0, v.size() - 1); }
  /*
   * @brief  更新一個 leaf `idx` = `val` 和 `idx` 的 dfs 線。
   */
  void add_node(const int& idx, const int& val) { add_node(idx, val, 1, 0, n - 1); }
  /*
