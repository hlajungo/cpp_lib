...
decode (block input[],
        int len,
        FILE* ptr); // Function used to decode Hamming code
void
encode (char* input, int len, FILE* ptr); // Function used to encode plaintext
void
printBlock (block i); // Function used to pretty print a block
bit
getBit (block b, int i); // Function used to get a specific bit of a block
bit
getCharBit (char b, int i); // Function used to get a specific bit of a char
block
toggleBit (block b, int i); // Function used to toggle a specific bit of a block
block
modifyBit (block n,
           int p,
           bit b); // Function used to modify a bit to a specific value
char
modifyCharBit (
    char n,
    int p,
    bit b); // Function used to modify a bit of a char to a specific value
int
multipleXor (
    int* indicies,
    int len); // Function used to XOR all the elements of a list together (used to locate error and determine values of parity bits)
int
inList (char** list,
        char* testString,
        int listLen); // Function used to check if a test string is in a list

void
usage (char* path)
{
  printf (
      "\nUsage: %s [COMMANDS]\n\n\tencode \tEncodes plaintext to hamming "
      "code\n\t\t-i \"FILENAME\" \tReads plaintext from file FILENAME (default "
      "is \"in.txt\")\n\t\t-o \"TEXT\" \tTEXT is read as plaintext input (file "
      "input is default)\n\t\t-o \"FILENAME\" \tOutputs hamming code to file "
      "FILENAME (default is \"out.hm\")\n\n\tdecode \tDecodes hamming code and "
      "prints original plaintext\n\t\t-i \"FILENAME\" \tReads hamming code "
      "from file FILENAME (default is \"out.hm\")\n\t\t-o \"FILENAME\" "
      "\tOutputs hamming code to file FILENAME (default is \"out.txt\")\n\n",
      path);
}
...
main (int argc, char** argv)
{
  ...
  if (strcmp (command, "decode") == 0)
  {
    ...
    {
      printf (
          "File \"%s\" cannot be opened (does the file exist?). Aborting.\n",
          rfilename);
      return 1;
    }
  ...
  if (strcmp (command, "encode") == 0)
  {
    ...
    if (inputIndex)
    {
      ...
      {
        printf (
            "-i (input file) and -t (text input) options cannot be used in "
            "conjunction. Aborting");
        return 1;
      }
...
void
encode (char* input, int len, FILE* ptr)
{
  ...
  for (int i = 0; i < blocks + 1; i++)
  {
    ...
    for (int j = 0; j < bits; j++)
    {
      ...
      if ((j & (j - 1)) == 0)
      { // Check if j is a power of two or 0
        skipped++;
        continue;
      }
      ...
      if (i != blocks)
      {
        ...
        thisBit = currentBit < len * sizeof (char) * 8
                      ? getCharBit (input[currentChar],
                                    currentBit - currentChar * 8)
                      : 0;
      }

      else
      {
        thisBit = getBit (len / 8,
                          j - skipped + (sizeof (block) * 8 - messageBits));
      }
    ...
    { // skip bit 0
      ...
      thisBlock
          = modifyBit (thisBlock,
                       ...
                       getBit (parityBits, sizeof (block) * 8 - skipped + k));
    }
...
void
decode (block input[], int len, FILE* ptr)
{
  ...
  for (int b = 0; b < (len / sizeof (block)); b++)
  {
    ...
    if (errorLoc)
    {
      ...
      if (!(onCount & 1 ^ getBit (input[b], 0)))
      { // last bit of onCount (total parity) XOR first bit of block (parity bit)
  ...
  for (int i = 0; i < len / sizeof (block); i++)
  {
    ...
    for (int j = 0; j < bits; j++)
    {
      ...
      if (i != len / sizeof (block) - 1)
      {
        ...
        output[currentChar]
            = modifyCharBit (output[currentChar],
                             currentBit - currentChar * sizeof (char) * 8,
                             thisBit);
      }
      ...
      {
        chars = modifyBit (
            chars, j - skipped + (sizeof (block) * 8 - messageBits), thisBit);
      }
    }
  }
...
int
multipleXor (int* indicies, int len)
{
  int val = indicies[0];
  for (int i = 1; i < len; i++)
  {
...
block
modifyBit (block n, int p, bit b)
{
  return ((n & ~(1 << (sizeof (block) * 8 - 1 - p)))
          | (b << (sizeof (block) * 8 - 1 - p)));
}

char
modifyCharBit (char n, int p, bit b)
{
  return ((n & ~(1 << (sizeof (char) * 8 - 1 - p)))
          | (b << (sizeof (char) * 8 - 1 - p)));
}

bit
getBit (block b, int i)
{
  return (b << i) & (int)pow (2, (sizeof (block) * 8 - 1));
}

bit
getCharBit (char b, int i)
{
  return (b << i) & (int)pow (2, (sizeof (char) * 8 - 1));
}

block
toggleBit (block b, int i)
{
  return b ^ (1 << i);
}

int
inList (char** list, char* testString, int listLen)
{
